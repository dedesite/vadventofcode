		...
		for y in dir {
			for x in dir {
				...
				if !(x == 0 && y == 0 && z == 0) {
					...
					neighbor_pos := '${x + x_pos},${y + y_pos},${z + z_pos}'
...
fn run_cycle(grid map[string]bool) map[string]bool {
	...
	for cube_pos, is_active in grid {
		if is_active {
			x_pos, y_pos, z_pos := get_pos(cube_pos)
			for z in dir {
				for y in dir {
					for x in dir {
						str_pos := '${x + x_pos},${y + y_pos},${z + z_pos}'
						next_cycle_grid[str_pos] = calculate_cube_new_state(str_pos, grid)
					}
				}
			}
		}
...
for y, line in initial_state {
	for x, state in line {
		grid['$x,$y,$z'] = if state == `#` { true } else { false }
	}
}
